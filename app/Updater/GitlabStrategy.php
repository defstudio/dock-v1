<?php
    ...
    class GitlabStrategy implements StrategyInterface{
        ...
        public function getCurrentRemoteVersion(Updater $updater){
            ...
            } else if($this->getStability() === self::UNSTABLE){
        ...
        public function setStability($stability){
            if($stability !== self::STABLE && $stability !== self::UNSTABLE && $stability !== self::ANY){
                throw new InvalidArgumentException('Invalid stability value. Must be one of "stable", "unstable" or "any".');
            }
        ...
        protected function getDownloadUrl(array $package){
            $baseUrl = preg_replace('{\.git$}', '', $package['packages'][$this->getPackageName()][$this->remoteVersion]['source']['url']);
            return sprintf('%s/releases/download/%s/%s', $baseUrl, $this->remoteVersion, $this->getPharName());
        }
    }
