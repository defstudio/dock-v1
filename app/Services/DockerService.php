...
class DockerService
{
    ...
    {
        if (!empty($this->networks[$internal_name])) {
            throw new DuplicateNetworkException("Duplicate network: ".$internal_name);
        }
    ...
    {
        if (!empty($this->networks[$network_name])) {
            throw new DuplicateNetworkException("Duplicate network: ".$network_name);
        }
    ...
    {
        if (!empty($this->containers[$container->service_name()])) {
            throw new DuplicateServiceException("Duplicate service: ".$container->service_name());
        }
    ...
    private function publish_docker_compose()
    {
        ...
        if (Storage::disk('cwd')->exists('custom-compose.yml')) {
            $docker_compose = [
                    'include' => [
                        'custom-compose.yml',
                    ],
    ...
    {
        if (empty($this->containers[$service_name])) {
            throw new DockerServiceNotFoundException("Service $service_name not found");
        }
